...
import { Accordion, AccordionDetails, AccordionSummary, Card, Grid, useMediaQuery, useTheme } from "@mui/material";
import ExpandMoreIcon from "@mui/icons-material/ExpandMore";

export default function ProgressCard({
	size, icon, name, children,
}: { size: { xs: number; sm: number }, icon: any, name: string, children: React.ReactNode }) {
	...
	return (
		<Grid item xs={ size.xs } sm={ size.sm }>
			<Card id={ `skills-${ name }` } sx={ { padding: "0rem" } }>
				<Accordion
					expanded={ expanded }
					onChange={ isSmall ? handleChange : () => {return;} }>
					<AccordionSummary
